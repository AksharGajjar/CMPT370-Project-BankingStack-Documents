...
- unit test #3: userFileCreation (storage system testing. Test to see if a file is created properly by checking the categories stored in the file.)
...
| UT#1 | Unit Test | register() | This function takes in the username, email, name, and password. It then uses that to create a user profile JSON. | username, email, name, and password | Successful if it is able to handle all possible invalid inputs safely and if valid inputs are provided it creates a json object |
| UT#2 | Unit Test | login() | The login function takes in username and password and once the login button is clicked the function will login user and in the back end the function will provide the user_id which will be used by other functions to access that users data. Provide valid and invalid data to create multiple test steps | username, password | Successful if it shows proper errors messages when invalid inputs are entered. And if valid inputs are entered it provides the correct user_id |
| UT#3 | Unit Test | userFileCreation() | Obtain data from register function, start parsing data, creates a JSON object. Check JSON object formatting and check the validity of the format. Once the file is created check file name, once data is added in the file verify that the file is not empty. | filename, username, email, password, name | Successful if json file is created appropriately. Successful if it handles errors successfully. | 
| UT#4 | Unit Test | addAccount() | Test by providing faulty account information, provide proper data, check and make sure the access_token is obtained once the addAccount performs the API contact. | bank account login credentials, access_token | Successful if it handles API responses appropriately in positive and negative cases. |
| UT#5 | Unit Test | removeAccount() | Test by entering account data for account that doesn't exist. Enter proper account credential. Test by mismatching input data for instance account_owner doesn't match the actual account. | account_name, access_token, account_owner | If the account credentials are invalidated and deleted from files when provided info is valid then it's successful. If an exception is thrown when false or mismatched data is provided then the test is successful. |
